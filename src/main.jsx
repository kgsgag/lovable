...
function App() {
  ...
  return <div className="app">
    ...
    <section id="cart" className="cart-section"><div><p className="eyebrow">SEU PEDIDO</p><h2>Carrinho</h2>{cart.length===0?<p className="empty">Seu carrinho está vazio. Adicione algo que você goste.</p>:cart.map(i=><div className="cart-item" key={i.id}><span className="mini-art">{i.emoji}</span><div className="cart-name"><b>{i.name}</b><span>{money(i.price)}</span></div><div className="qty"><button aria-label="Diminuir quantidade" onClick={()=>change(i.id,-1)}><Minus size={14}/></button><span>{i.quantity}</span><button aria-label="Aumentar quantidade" onClick={()=>change(i.id,1)}><Plus size={14}/></button></div><button className="remove" aria-label={`Remover ${i.name}`} onClick={()=>setCart(c=>c.filter(x=>x.id!==i.id))}><Trash2 size={16}/></button></div>)}</div><aside className="summary"><p>Total</p><h3>{money(total)}</h3><button className="primary full" disabled={!cart.length} onClick={checkout}><PackageCheck size={18}/> Finalizar pedido</button></aside></section></main>
